"""Scrape the FB Marketplace inbox and yield new inbound messages."""
...
def _read_latest_inbound(page: Page, tid: str) -> str | None:
    ...
    # Messages render as div[dir="auto"]. The latest one is the newest.
    # In an unread thread, the latest non-empty one is the new inbound message.
    msgs = []
    ...
    # Filter out the textbox placeholder text and system noise
...
def poll_inbox(ctx: BrowserContext) -> tuple[list[InboundMessage], int]:
    """Returns (unread inbound messages, total threads visible on inbox)."""
    page = ctx.new_page()
    ...
    try:
        ...
        # Also wait for the thread list to actually populate
        try:
            page.locator('a[href*="/marketplace/t/"], a[href*="/messages/t/"]').first.wait_for(timeout=15_000)
        except Exception:
            pass

        if "login" in page.url.lower():
            raise RuntimeError(
                "Not logged in. Run `python -m src.main login` to authenticate."
            )
        ...
        for t in threads[:5]:
            print(f"    - tid={t['thread_id'][:14]} unread={t['unread']} who={t['counterparty']!r} listing={t['listing']!r}")
